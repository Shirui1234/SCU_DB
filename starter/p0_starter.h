...
namespace scudb {
	...
	public:
		...
		T GetElement(int i, int j) const override {
			if (i < 0 || i >= this->rows_ || j < 0 || j >= this->cols_) throw std::out_of_range("RowMatrix::GetElement()");
		...
		void SetElement(int i, int j, T val) override {
			if (i < 0 || i >= this->rows_ || j < 0 || j >= this->cols_) throw std::out_of_range("RowMatrix::SetElement()");
			else data_[i][j] = val;
		}
		...
		void FillFrom(const std::vector<T> &source) override {
			if (source.size() != this->rows_ * this->cols_) throw std::out_of_range("RowMatrix::FillFrom()");
			else {
	...
	public:
		...
		static std::unique_ptr<RowMatrix<T>> Add(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
			// TODO(P0): Add implementation
			if (matrixA->GetRowCount() != matrixB->GetRowCount() || matrixA->GetColumnCount() != matrixB->GetColumnCount() || !matrixA || !matrixB) {
		...
		static std::unique_ptr<RowMatrix<T>> GEMM(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB,
			...
			else return Add(Multiply(matrixA, matrixB).get(), matrixC);
		}
		
		//print the matrix
		static void Show(const RowMatrix<T> &m)
		{
